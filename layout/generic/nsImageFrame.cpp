...
bool nsDisplayImage::CreateWebRenderCommands(
  ...
  if (nsImageMap* map = frame->GetImageMap();
    ...
    // We can't draw some focus areas (in particular, PolyArea would be
    // somewhat hard to do), and cropped XUL images need the regular painting
    // path so that the source rect is honored.
    return false;
  }
